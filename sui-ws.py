#!/usr/bin/env python
"""
HACK TO USE WEBSOCKETS API (annoying, but only way to grab all
the preview images and make an animated WEBP out of them)
    ...
    async with connect(f"{baseurl}/API/GenerateText2ImageWS",
        ...
        while True:
            try:
                ...
                if 'gen_progress' in response:
                    if 'preview' in response['gen_progress']:
                        progress.append(response['gen_progress']['preview'])
                elif 'image' in response:
                    # optionally save previews as animated webp
                    frames = list()
                    frame_num = 0
                    for preview in progress:
                        ...
                        frame = Image.open(img_stream)
                        frame = frame.resize([ frame.width*4, frame.height*4])
